...
if (opts.list || !opts.mode) {
  ...
  if (!opts.mode && !opts.list) fail("Nothing installed: pass --user or --project <dir>.");
  if (!opts.mode) {
...
const destRoot =
  opts.mode === "user"
    ...
    : path.join(path.resolve(opts.target ?? process.cwd()), ".claude", "skills");
...
if (installed.length && !opts.dryRun) {
  console.log(
    `\nRestart your agent session to pick them up. Verify with /skills in Claude Code,\n` +
      `or ask: "which MarkLogic skills do you have?"`
  );
}
